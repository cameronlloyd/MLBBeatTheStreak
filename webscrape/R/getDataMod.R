...
scrapeGame <-function(game.link,month) {
  ...
  for (batter in awayBatters) {
    result = rbind(result,createRow(batter,FALSE,homePitcherData,hitTable,month))
  }
  print("scraping the home batters now")
  for (batter in homeBatters) {
    result = rbind(result,createRow(batter,TRUE,awayPitcherData,hitTable,month))
  }
...
createRow <-function(batter.link,home,pitcherData,hitTable,month) {
  ...
  otherInfo.Dat = data.frame(pitcherInMonth = c(pitcherInMonth = c(month.Career.Dat[1]),batterInMonth= c(month.Career.Dat[2])))
...
cleanData <- function(data) {
  ...
  names(data)[25:29] = c('hit','atBats','pitcherAvgInMonth','batterAvgInMonth','Date')
